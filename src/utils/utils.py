...
def save_results_to_txt(output_dir, **kwargs):
    ...
    with open(Path(output_dir, "results.txt"), "w") as f:
        f.write("#"*50 + "\n")
        f.write(f"# {'':14} Evaluation Report {'':13} #" + "\n")
        f.write("#"*50 + "\n")
        if "classification" in output_dir:
            f.write(f"| {'':18} CLASSES {'':19} |" + "\n")
            f.write("-"*50 + "\n")
            if "perc_unk" in kwargs:
                f.write(f"| {'Unknown (%)':<22} | {kwargs['perc_unk']*100:<21.4f} |" + "\n")
                f.write("-"*50 + "\n")
            if "accuracy" in kwargs:
                f.write(f"| {'Accuracy (%)':<22} | {kwargs['accuracy']*100:<21.4f} |" + "\n")
                f.write("-"*50 + "\n")
            if "f1" in kwargs:
                f.write(f"| {'F1-Score (%)':<22} | {kwargs['f1']*100:<21.4f} |" + "\n")
                f.write("-"*50 + "\n")
            if "balanced_acc" in kwargs:
                f.write(f"| {'Balanced Accuracy (%)':<22} | {kwargs['balanced_acc']*100:<21.4f} |" + "\n")
                f.write("-"*50 + "\n")
            if "cm" in kwargs:
                for i in range(kwargs["cm"].shape[1]):
                    if i == 0:
                        f.write(f"| {'Confusion Matrix':<22} | {str(kwargs['cm'][i].tolist()):<21} |" + "\n")
                    else:
                        f.write(f"| {'':<22} | {str(kwargs['cm'][i].tolist()):<21} |" + "\n")
                f.write("-"*50 + "\n")
        else:
            f.write(f"| {'':18} CONCEPTS {'':18} |" + "\n")
            f.write("-"*50 + "\n")
            if "perc_unk" in kwargs:
                f.write(f"| {'Unknown (%)':<22} | {kwargs['perc_unk']*100:<21.4f} |" + "\n")
                f.write("-"*50 + "\n")
            if "concepts_bacc" in kwargs:
                f.write(f"| {'Balanced Accuracy ':<22} | {kwargs['concepts_bacc']*100:<21.4f} |" + "\n")
                f.write("-"*50 + "\n")
            if "concepts_f1" in kwargs:
                f.write(f"| {'F1-score':<22} | {kwargs['concepts_f1']*100:<21.4f} |" + "\n")
                f.write("-"*50 + "\n")
            if "exact_match" in kwargs:
                f.write(f"| {'Exact Match':<22} | {kwargs['exact_match']*100:<21.4f} |" + "\n")
                f.write("-"*50 + "\n")
            if "concepts_f1_per_concept" in kwargs:
                for idx, value in enumerate(kwargs['concepts_f1_per_concept']):
                    if idx == 0:
                        f.write(f"| {'F1-score (per concept)':<22} | Concept {idx+1}: {value*100:<10.4f} |" + "\n")
                    else:
                        f.write(f"| {'':<22} | Concept {idx+1}: {value*100:<10.4f} |" + "\n")
                f.write("-"*50 + "\n")
            if "ml_cm" in kwargs:
                for i in range(kwargs['ml_cm'].shape[0]):
                    if i == 0:
                        f.write(f"| {'Confusion Matrix':<22} | {str(kwargs['ml_cm'][i,:].tolist()):<21} |" + "\n")
                    else:
                        f.write(f"| {'':<22} | {str(kwargs['ml_cm'][i,:].tolist()):<21} |" + "\n")
                f.write("-"*50 + "\n")
...
def print_config(
    ...
    Args:
        ...
        save_to_file (bool, optional): Whether to export config to the hydra output folder. Default is ``False``.
    """
